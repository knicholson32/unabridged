...
export const load = async ({ params }) => {
	...
	series.books.sort((a, b) => {
		if (a.series_sequence !== null && b.series_sequence !== null)
			return a.series_sequence > b.series_sequence ? 1 : -1;
	...
	for (const book of series.books) {
		...
		for (const author of book.authors)
			if (authors.find((a) => a === author.name) === undefined) authors.push(author.name);
		for (const narrator of book.narrators)
			if (narrators.find((n) => n === narrator.name) === undefined) narrators.push(narrator.name);
	...
	const collectionURL =
		series.plexKey === null
			? null
			: await Plex.collections.getCollectionWebURL(
					series.plexKey,
