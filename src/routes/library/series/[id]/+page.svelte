...
<White>
		...
		<ol role="list" class="mx-auto flex w-full space-x-4 px-4 sm:px-6 lg:px-8">
			...
			<li class="flex">
				<div class="flex items-center">
					...
					<a
						href="/library/series/{data.series.id}"
	...
	<div class="">
		<div
			class="relative flex flex-col p-4 {coverBright ? 'text-gray-800' : 'text-white'}"
			style="background-color: {coverHEX};"
		>
			<div class="absolute right-4 top-4 {coverBright ? '' : 'dark'}">
				<div class="relative">
					<button
						on:click={toggleSeriesMenu}
						bind:this={seriesMenuButton}
						class="px-1 py-1 text-gray-700 transition-colors duration-200 rounded-lg dark:text-gray-300 hover:bg-gray-100/70 dark:hover:text-gray-700"
					>
						<svg
            ...
            -->
					{#if seriesMenuVisible}
						...
						<div
							...
							use:EscapeOrClickOutside={{ except: seriesMenuButton, callback: closeSeriesMenu }}
								...
								<div class="py-1" role="none">
									{#if !data.series.books.every((b) => b.downloaded === true && b.processed === true)}
										<button
											...
											class="unstyled whitespace-nowrap text-left font-medium w-full hover:bg-gray-100 hover:text-gray-900 text-gray-700 block px-4 py-2 text-sm !no-underline"
											role="menuitem"
											tabindex="-1"
										>
											Download All
			...
			<div>
				<h1 class="font-medium text-2xl uppercase tracking-widest font-serif">
					{data.series.title}
					<span class="block sm:inline-flex font-sans text-sm text-opacity-30"
						>{data.series.books.filter((b) => b.downloaded === true && b.processed === true)
							.length}/{data.series.books.length}
						{helpers.basicPlural('Book', data.series.books.length)} Downloaded</span
					>
				</h1>
			</div>
			<div class="text-xs opacity-80">
				By {data.authors.join(', ')} | Narrated By {data.narrators.join(', ')}
			</div>
			...
			<div>
				<Rating
					...
					rating={data.weightedAvgRating.value}
					numReviews={data.weightedAvgRating.totalReviews}
				/>
			</div>
		</div>
