...
<Gray class="px-4 pt-5 pb-5 mx-auto">
	...
	<div class="sm:flex sm:items-center sm:justify-between">
		<div>
			<div class="flex items-center gap-x-3">
				...
				<span
					class="px-3 py-1 text-xs align-middle text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400"
				>
					<span class="inline-flex leading-5"
						>{data.bookCount} {helpers.basicPlural('book', data.bookCount)}</span
					>
	...
	<div class="mt-6 md:flex md:items-center md:justify-between">
		...
		>
			<a
				href={groupBySeriesURL}
				class="{params.group === 'series'
					? 'bg-gray-100 dark:bg-gray-800'
					: 'hover:bg-gray-100 dark:hover:bg-gray-800'} px-5 py-2 text-xs sm:text-sm font-medium text-gray-600 transition-colors duration-200 dark:text-gray-300"
			>
				Series
			</a>
			...
			<a
				href={groupByAuthorURL}
				class="{params.group === 'author'
					? 'bg-gray-100 dark:bg-gray-800'
					: 'hover:bg-gray-100 dark:hover:bg-gray-800'} px-5 py-2 text-xs sm:text-sm font-medium text-gray-600 transition-colors duration-200 dark:text-gray-300"
			>
				Author
			...
			<a
				href={groupByNarratorURL}
				class="{params.group === 'narrator'
					? 'bg-gray-100 dark:bg-gray-800'
					: 'hover:bg-gray-100 dark:hover:bg-gray-800'} px-5 py-2 text-xs sm:text-sm font-medium text-gray-600 transition-colors duration-200 dark:text-gray-300"
			>
				Narrator
		...
		<SortBy href={groupSortURL} selected={true} direction={params.groupOrder} />
	...
	{#if data.bookCount !== 0}
		<div class="flex flex-col mt-6">
			<div class="-mx-0 -my-2 sm:-mx-0 lg:-mx-0">
				<!-- overflow-x-auto -->
				<div class="inline-block min-w-full max-w-full w-full py-2 align-middle md:px-0 lg:px-0">
					<div class="border overflow-clip border-gray-200 dark:border-gray-700 md:rounded-lg">
						<!-- overflow-hidden -->
						<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
							<thead class="bg-gray-100 dark:bg-gray-800 shadow-sm">
								<tr>
									<th
										scope="col"
										class="py-3.5 {tableXPaddingLeft} text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400"
									>
										<SortBy
											href={titleSortURL}
											selected={params.sortCategory === 'title'}
											direction={params.order}>Title</SortBy
										>
									</th>

									<th
										scope="col"
										class="{tableXPadding} py-3.5 w-[1%] hidden md:table-cell text-sm font-normal focus:outline-none select-none rtl:text-right text-gray-500 dark:text-gray-400"
									>
										<SortBy
											href={orderSortURL}
											selected={params.sortCategory === 'order'}
											direction={params.order}>Order</SortBy
										>
									</th>

									<th
										scope="col"
										class="{tableXPadding} py-3.5 hidden lg:table-cell text-sm font-normal text-center rtl:text-right text-gray-500 dark:text-gray-400"
									>
										Genres
									</th>
									...
									<th
										scope="col"
										class="{tableXPadding} py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400"
									>
										About
									</th>
									...
									<th
										scope="col"
										class="{tableXPadding} py-3.5 w-[1%] text-sm font-normal text-center rtl:text-right text-gray-500 dark:text-gray-400"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											fill="none"
								...
								{#each data.series as series}
									<thead class="bg-gray-50 dark:bg-gray-800 sticky top-[63px]">
										<tr>
											<th
												...
												scope="col"
												colspan={numCols - 1}
												class="py-3.5 {tableXPaddingLeft} text-sm text-left rtl:text-right font-medium text-gray-800 dark:text-white"
											>
												...
												<span class="font-normal text-xs text-gray-400 dark:text-gray-500">
													<span title={series.authors.join(', ')}
														>{helpers.joinWithLimit(series.authors, 2)}</span
													>
													|
													{series.numBooks}
													...
													<svg
														...
														viewBox="0 0 20 20"
														fill="currentColor"
														class="w-5 h-5 inline-flex pb-0.5"
													>
														<path
															fill-rule="evenodd"
											...
											<td class="{tableXPadding} py-2 text-sm whitespace-nowrap">
												...
												{:else}
													<button
														type="button"
														on:click={() => {
															console.log(fetch(`/api/library/download/series/${series.id}`));
														}}
													>
														<svg
															xmlns="http://www.w3.org/2000/svg"
											...
											<td class="{tableXPadding} py-2 text-sm whitespace-nowrap">
												<div class="flex items-center justify-center">
													{#each series.books
														.flatMap((b) => b.sources)
														.filter((e, p, s) => s.findIndex((e2) => e.id === e2.id) === p) as source}
														<a
															href="/sources/{source.id}"
															class="object-cover w-6 h-6 -mx-1 border-2 border-white rounded-full overflow-hidden dark:border-gray-700 shrink-0"
														>
									...
									>
										{#each series.books as book}
											<Book {book} groupBy={'series'} {tableXPadding} {tableXPaddingLeft} />
										{/each}
									</tbody>
								{/each}
			...
			<div class="flex flex-col w-full max-w-sm px-4 mx-auto">
				...
				{#if params.search !== ''}
					<p class="mt-2 text-gray-500 dark:text-gray-400">
						Your search “{params.search}” did not match any books. Please try again.
					</p>
				{:else}
					<p class="mt-2 text-gray-500 dark:text-gray-400">
						No books in library. Add another account, or sync your current accounts.
					</p>
				{/if}
	...
	<div class="mt-6 sm:flex sm:items-center sm:justify-between">
		<div class="text-sm text-gray-500 dark:text-gray-400">
			Page <span class="font-medium text-gray-700 dark:text-gray-100"
				>{data.page == undefined ? '?' : data.page + 1} of {data.pages}</span
			>
		</div>
