...
export async function load({ url, depends }) {
	...
	const fuseOptionsSingleBooks = {
		...
		// ignoreFieldNorm: false,
		// fieldNormWeight: 1,
		keys: ['title', 'authors.name', 'narrators.name', 'genres.tag']
	};
	...
	if (params.group === 'series') {
		...
		let series = seriesRaw as ((typeof seriesRaw)[0] & {
			authors: string[];
			narrators: string[];
		...
		for (const s of series) {
			...
			for (const book of s.books) {
				for (const author of book.authors)
					if (s.authors.includes(author.name) === false) s.authors.push(author.name);
				for (const narrator of book.narrators)
					if (s.narrators.includes(narrator.name) === false) s.narrators.push(narrator.name);
				s.numBooks++;
		...
		const authors = authorsRaw as ((typeof authorsRaw)[0] & {
			narrators: string[];
			runTimeMinutes: number;
			numBooks: number;
		})[];
		for (const a of authors) {
			...
			for (const book of a.books) {
				for (const narrator of book.narrators)
					if (a.narrators.includes(narrator.name) === false) a.narrators.push(narrator.name);
				bookCount++;
				a.numBooks++;
				runTimeMin += book.runtime_length_min ?? 0;
				book.rating = book.rating.toNumber() as unknown as Decimal;
		...
		const narrators = narratorsRaw as ((typeof narratorsRaw)[0] & {
			authors: string[];
			runTimeMinutes: number;
			numBooks: number;
		})[];
		for (const n of narrators) {
			...
			for (const book of n.books) {
				for (const author of book.authors)
					if (n.authors.includes(author.name) === false) n.authors.push(author.name);
				bookCount++;
				n.numBooks++;
				runTimeMin += book.runtime_length_min ?? 0;
				book.rating = book.rating.toNumber() as unknown as Decimal;
