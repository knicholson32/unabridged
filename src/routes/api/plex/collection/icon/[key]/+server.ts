...
export const GET = async ({ setHeaders, params, request }) => {
  ...
  if (plexSettings['plex.enable'] === false) return API.response._400({ message: 'Plex is not enabled' });
  ...
  try {
    ...
    if (debug && response.status === 401) {
      ...
      return json({ status: 401, ok: false, message: 'Plex is not authorized' } satisfies API.Error, { status: 401 });
    }

    const encoding =
      request.headers.get('accept-encoding')?.includes('bsr') ? 'br' :
