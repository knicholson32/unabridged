...
export function GET({ request, url }) {
	...
	if (problematicTargets.length > 0)
		return API.response._400({
			message: `The following targets do not exist: '${problematicTargets.join(
				', '
			)}'. Options: '${Event.Progress.Names.join(', ')}'`
		});
	...
	const cleanup = () => {
		for (const e of Event.Progress.Names)
			if (callbackMap[e] !== undefined) progress.off(e, callbackMap[e]);
	};
