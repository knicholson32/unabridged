...
export const load = async ({ params, url }) => {
	...
	const plexDetails = async () => {
		...
		if (
			settingValues['plex.enable'] === true &&
			settingValues['plex.address']?.length > 0 &&
			settingValues['plex.token']?.length > 0
		) {
			// Test the Plex connection
			const results = await Plex.testPlexConnection(
				settingValues['plex.address'],
				settingValues['plex.token']
			);
			...
			if (results.success === true) {
				...
				const results = await Plex.getLibraries(
					settingValues['plex.address'],
					settingValues['plex.token']
				);
				console.log(results);
				...
				if (collectBy === CollectionBy.series) {
					...
					const collectionsResult = await prisma.series.findMany({
					...
					for (const collection of collectionsResult) {
						...
						const url = await Plex.collections.getCollectionWebURL(
							collection.plexKey,
							settingValues['plex.address'],
							machineId
						);
						...
						const collectionWithDetails = {
							...collection,
							...{ icon, url }
						} satisfies CollectionDetails;
						for (const b of collectionWithDetails.books)
							b.rating = b.rating.toNumber() as unknown as Decimal;
		...
		if (signedIn) {
			...
			for (const section of sections) {
				...
				if (section.type === 'artist')
					filteredSections.push({ value: section.uuid, title: section.title });
				// Otherwise add it as an un-selectable option
				else
					filteredSections.push({
						value: section.uuid,
						title: `${section.title} (${helpersPublic.capitalizeFirstLetter(
							section.type
						)} Library)`,
						unset: true
					});
			...
			if (settingValues['plex.library.name'] === '' || settingValues['plex.library.id'] === '') {
				...
				filteredSections.push({
					value: settingValues['plex.library.id'],
					title: settingValues['plex.library.name'],
					unset: true
				});
			}
		}
...
export const actions = {
	...
	updatePlexIntegration: async ({ request }) => {
		...
		if (signIntoPlex !== null) {
			...
			const additionalURLParams = {
				...
				forwardUrl: `${helpers.removeTrailingSlashes(
					process.env.ORIGIN ?? '127.0.0.1'
				)}/settings/plex/oauth/${pinId}`
			};
			...
			if (plexEnable ?? constSettings['plex.enable']) {
				// Test the plex connection
				const results = await Plex.testPlexConnection(
					address ?? constSettings['plex.address'],
					token ?? constSettings['plex.token']
				);
				if (results.success === true) {
					...
					if (library !== undefined) {
						...
						const results = await Plex.getLibraries(
							address ?? (await settings.get('plex.address')),
							token ?? (await settings.get('plex.token'))
						);
						...
						if (results === null || results.length === 0)
							return {
								...
								success: false,
								message: 'Could not get library information'
							};
						...
						if (results[index].type !== 'artist')
							return {
								action: '?/updatePlexIntegration',
								name: 'plex.library.id',
								success: false,
								message: 'Invalid library type. Please select a valid option.'
							};
	...
	testPlexIntegration: async () => {
		...
		const results = await Plex.testPlexConnection(
			addressAndToken['plex.address'],
			addressAndToken['plex.token']
		);
		return {
			action: '?/updatePlexIntegration',
			invalidatedParams: false,
			name: results.source,
			success: results.success,
			message: results.message
		};
	...
	clearPlexIntegration: async ({ request }) => {
		...
		const constSettings = await settings.getMany(
			'system.debug',
			'plex.collections.by',
			'plex.address',
			'plex.token',
		...
		const clearCollections = (data.get('plex.collections.delete') ?? undefined) as
			| undefined
			| string;
		if (clearCollections !== undefined && clearCollections === 'true') {
			...
			if (collectBy === CollectionBy.series) {
				...
				const series = await prisma.series.findMany({
					where: { plexKey: { not: null } },
					select: { plexKey: true, title: debug !== 0 }
				});
				// Loop through the collections
				for (const s of series) {
					if (s.plexKey === null) continue;
					const result = await Plex.collections.deleteCollection(
						s.plexKey,
						plexURL,
						plexToken,
						debug
					...
					if (debug)
						console.log(
							`Collections: Removing '${s.title}' - ${s.plexKey} : ${
				...
				if (constSettings['plex.collections.singlesKey'] !== '') {
					const r = await Plex.collections.deleteCollection(
						constSettings['plex.collections.singlesKey'],
						plexURL,
						plexToken,
						debug
					);
	...
	updatePlexAPI: async ({ request }) => {
		...
		const plexScanTimeout = (data.get('plex.library.autoScan.timeout') ?? undefined) as
			| undefined
			| string;
		if (plexScanTimeout !== undefined)
			await settings.set('plex.library.autoScan.timeout', parseInt(plexScanTimeout));
	},
	updatePlexLibrary: async ({ request }) => {
		...
		if (autoScan !== undefined)
			await settings.set('plex.library.autoScan.enable', autoScan === 'true');

		const autoScanDelay = (data.get('plex.library.autoScanDelay') ?? undefined) as
			| undefined
			| string;
		if (autoScanDelay !== undefined)
			await settings.set('plex.library.autoScan.delay', parseInt(autoScanDelay));

		const scheduled = (data.get('plex.library.scheduled') ?? undefined) as undefined | string;
		if (scheduled !== undefined)
			await settings.set('plex.library.autoScan.scheduled', scheduled === 'true');
	},
	updatePlexCollections: async ({ request }) => {
		const data = await request.formData();

		const collectionsEnable = (data.get('plex.collections.enable') ?? undefined) as
			| undefined
			| string;
		if (collectionsEnable !== undefined)
			await settings.set('plex.collections.enable', collectionsEnable === 'true');
		...
		if (collectBy !== undefined) {
			if (collectBy !== CollectionBy.series) {
				return {
					...
					success: false,
					message: 'Invalid collection type. Please choose a valid selection.'
				};
			} else await settings.set('plex.collections.by', collectBy);
		}

		const groupSingles = (data.get('plex.collections.groupSingles') ?? undefined) as
			| undefined
			| string;
		if (groupSingles !== undefined)
			await settings.set('plex.collections.groupSingles', groupSingles === 'true');
	},
	generateCollections: async ({ request }) => {
		const result = await Plex.scanAndGenerate();
		return {
			action: '?/generateCollections',
			name: 'plex.scanAndGenerate',
			success: result === ScanAndGenerate.NO_ERROR,
			message: scanAndGenerateToStringLong(result)
		};
	}
};
