...
<div class="flex items-center border rounded-md">
	...
	<div class="flex flex-col items-start content-start ml-2">
		...
		<div class="text-gray-400 text-xxs leading-3">
			Downloaded {collection.books.filter((b) => b.downloaded).length} / {collection.books.length} books
		</div>
		<div class="text-gray-400 text-xxs leading-3">
			Processed {collection.books.filter((b) => b.processed).length} / {collection.books.length} books
		</div>
	</div>
	<div class="grow" />
	<div class="flex flex-col divide-y h-16 justify-center border-l ml-2">
		<VerifyButton
			click={trash}
			class="py-1.5 w-6 grow h-full flex items-center justify-center hover:bg-gray-100"
		>
			<svg
				slot="icon"
	...
	<div class="flex flex-col divide-y h-16 justify-center border-l">
		<a
			href={collection.url}
			target="_blank"
			class="p-1.5 grow flex items-center hover:bg-gray-100 rounded-tr"
		>
			<svg
				class="h-5 w-5"
				fill="none"
		...
		<a
			href="/library/series/{collection.books[0].seriesId}"
			target="_blank"
			class="p-1.5 grow flex items-center group hover:bg-gray-100 rounded-br"
		>
        ...
        {@html icons.chevronRight}
			...
			>
				<path
					d="M24.7,5C13.3,5,4,14.3,4,25.7V120h94.3c11.4,0,20.7-9.3,20.7-20.7V5H24.7z M59.5,81l-14.6,0.7l-0.4-6
