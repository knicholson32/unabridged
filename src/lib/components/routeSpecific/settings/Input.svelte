...
>
	<div class="-my-2 flex flex-col-reverse sm:flex-row sm:inline-flex sm:items-center">
		{#if leadingText !== null}
			<p
				class="mr-2 text-xxs font-mono select-none text-right {leadingText.error
					? 'text-red-400'
					: 'text-gray-400'}"
			>
				{leadingText.t}
		...
		<input
			...
			type="text"
			{name}
			class="block {small ? 'w-[12em]' : 'min-w-[16em]'} {mono
				? 'font-mono'
				: ''} rounded-md border-0 py-1.5 text-gray-900 shadow-sm placeholder:text-gray-400 ring-1 ring-inset {error
				? 'ring-red-300 focus:ring-red-600'
				: 'ring-gray-300 focus:ring-indigo-600'}  focus:ring-2 focus:ring-inset disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500 disabled:ring-gray-200 sm:text-sm sm:leading-6"
			{placeholder}
			bind:value
		/>
	</div>
