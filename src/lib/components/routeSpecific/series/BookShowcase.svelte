...
<div class="relative w-full flex flex-col">
	<div class="absolute left-0 right-0 top-0 bottom-0 shadow-md rounded-xl overflow-hidden bg-white">
		<div
			class="absolute left-0 right-0 top-0 bottom-0 brightness-200 saturate-100 opacity-30"
			style=" background-color: {book.cover?.hex_dom};"
		/>
	</div>
	<div class="relative flex w-full h-full pl-4">
		...
		<div class="flex-grow w-[1%] p-4 flex flex-col">
			<p class="text-black relative font-bold font-serif text-lg whitespace-nowrap">
				...
				<span class="text-xxs leading-7 font-sans font-normal text-gray-700 align-baseline ml-1"
					>Book {book.series_sequence}</span
				>
			...
			<p class="font-normal font-sans text-xxs text-gray-700">
				by {authors}, Narrated by {narrators}
			</p>
			{#if book.runtime_length_min !== null}
				<p class="font-normal font-sans text-xxs text-gray-700">
					{new helpers.RunTime({ min: book.runtime_length_min }).toFormat()} long
				</p>
			{/if}
			<Rating {rating} numReviews={book.num_ratings} />
			<p
				title={description}
				class="pt-0.5 text-xs h-[6.25rem] text-justify overflow-y-auto text-ellipsis"
			>
				{description}
			</p>
			<p class="flex space-x-1 pt-1 overflow-x-auto flex-nowrap select-none">
				{#each book.genres as genre}
					<span
						class="inline-flex whitespace-nowrap px-1.5 py-0.5 items-center text-xs font-normal rounded-full text-gray-800 gap-x-2 bg-white/60 dark:bg-gray-800"
						>{genre.tag}</span
					>
				{/each}
			</p>
