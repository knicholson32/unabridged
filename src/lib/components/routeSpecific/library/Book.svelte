...
<tr>
	<td
		class="{tableXPaddingLeft} {maxWidthTitle.total} py-2 text-sm font-medium whitespace-nowrap inline-flex space-x-3"
	>
		...
		<div class="{maxWidthTitle.text} truncate">
			<a href="/library/books/{book.asin}">
				<h2
					title={book.title}
					class="font-medium text-gray-800 text-ellipsis overflow-hidden dark:text-white"
				>
					{helpers.truncateString(book.title, truncateNum)}
				</h2>
			</a>
			{#if subTitle === 'runtime'}
				{#if book.runtime_length_min !== null}
					<p
						title="Playback time of {new helpers.RunTime({
							min: book.runtime_length_min
						}).toDirectFormat()}"
						class="text-sm font-normal text-gray-600 text-ellipsis overflow-hidden dark:text-gray-400"
					>
			...
			{:else if subTitle === 'series'}
				{#if book.series !== null}
					<p
						class="text-sm font-normal text-ellipsis overflow-hidden text-gray-600 dark:text-gray-400"
					>
	...
	<td
		title={book.genres.map((g) => g.tag).join(', ')}
		class="max-w-[150px] hidden lg:table-cell max-h-3 {tableXPadding} py-2 text-sm font-medium whitespace-nowrap table-cell truncate space-x-1"
	>
		{#each book.genres as genre}
			<span
				class="inline-flex px-1.5 py-0.5 items-center text-xs font-normal rounded-full text-emerald-500 gap-x-2 bg-emerald-100/60 dark:bg-gray-800"
				>{genre.tag}</span
			>
		{/each}
	</td>
	<td class="{tableXPadding} py-2 text-sm whitespace-nowrap">
		<div class="max-w-[150px]">
			{#if book.subtitle === null && book.description === null}
				<h4
					title="By {book.authors[0]?.name ?? 'unknown'}"
					class="text-gray-700 dark:text-gray-200 overflow-hidden truncate"
				>
					By {book.authors[0]?.name ?? 'unknown'}
				</h4>
				<p
					title="Narrated by {book.narrators[0]?.name ?? 'unknown'}"
					class="text-gray-500 dark:text-gray-400 overflow-hidden truncate"
				>
			...
			{:else if book.subtitle !== null && book.description === null}
				<h4 title={book.subtitle} class="text-gray-700 dark:text-gray-200 overflow-hidden truncate">
					{book.subtitle}
				</h4>
				<p
					title={book.authors[0]?.name ?? 'unknown'}
					class="text-gray-500 dark:text-gray-400 overflow-hidden truncate"
				>
		...
		<div class="flex items-center justify-center">
			{#each book.sources as source}
				<a
					...
					><img src="{source.profile_image_url}/56" alt="" /></a
				>
			{/each}
