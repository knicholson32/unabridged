...
<div class="flex flex-col justify-center">
	...
	{:else if state === States.PROGRESS}
		...
		<div
			class="shrink-0 text-xxs border border-gray-400 bg-white text-gray-800 rounded-md pl-2 flex items-center gap-1 truncate overflow-hidden"
		>
			Collecting
			<div class="transition-colors duration-100 p-0.5 bg-white text-white">
				<LoadingCircle {progress} {spin} theme={{ primary: 'white' }} spinnerOnly={true} />
			</div>
		</div>
	{:else if state === States.RESULT}
		{#if result === ScanAndGenerate.NO_ERROR || result === ScanAndGenerate.NO_ERROR_COLLECTIONS_DISABLED}
			<div
				title={scanAndGenerateToStringLong(result)}
				class="shrink-0 h-7 text-xxs border border-gray-400 bg-white text-gray-800 rounded-md px-2 flex items-center gap-1 truncate overflow-hidden"
			>
		...
		{:else}
			<div
				title={scanAndGenerateToStringLong(result)}
				class="shrink-0 h-7 text-xxs border border-red-600 bg-white text-red-600 rounded-md px-2 flex items-center gap-1 truncate overflow-hidden"
			>
