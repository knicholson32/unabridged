<script lang="ts">
	...
	const close = async () => {
		...
		if (
			...
			defAsNotification.settings.deleteOnlyAfterClose === true
		) {
	...
	const unpauseProgress = () => {
		...
		if (definition.type === 'alert') {
			...
			if (
				defAsNotification.settings.linger_ms !== undefined &&
				defAsNotification.settings.linger_ms !== 0
			) {
...
<div
	...
	on:mouseleave={unpauseProgress}
	in:transitionIn
	out:fade={{ duration: 200 }}
>
	{#if definition.type === 'alert'}
		<div
			class="overflow-hidden rounded-lg shadow-xl backdrop-blur-md {bg} ring-1 {ring} ring-opacity-5"
		>
			<div class="p-4">
				<div class="flex items-start select-none">
					<div class="flex-shrink-0">
						<svg
							class="h-6 w-6 {defAsAlert.settings.iconColor ?? defIconColor}"
							fill="none"
							viewBox="0 0 24 24"
							stroke-width="1.5"
							stroke="currentColor"
				...
				<div class="flex items-start select-none">
					<div class="flex-shrink-0">
						{#if defAsNotification.settings.iconURL !== undefined}
							<img
								class="h-6 w-6 rounded-sm overflow-hidden"
								src={defAsNotification.settings.iconURL}
								alt=""
							/>
						{:else}
							<svg
								class="h-6 w-6 {defAsNotification.settings.iconColor ?? defIconColor}"
								fill="none"
								viewBox="0 0 24 24"
								stroke-width="1.5"
								stroke="currentColor"
