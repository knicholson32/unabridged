<script lang="ts">
	// import type { ProgressEvents, Progress } from "$lib/types";
	...
	switch (themeDefault) {
		...
		case 'white':
			...
			themeClassesDefault =
				'bg-white text-gray-800 hover:bg-gray-100 hover:text-gray-900 disabled:text-gray-200 disabled:bg-gray-800 disabled:hover:bg-gray-800 focus-visible:outline-grey-500';
...
{#if spinnerOnly}
	<div class="{clazz} select-none transition-colors flex justify-center">
		<!-- <span class="flex-none"></span> -->
		<div class="relative flex-none w-5 h-5 align-middle overflow-hidden">
			{#if !spin}
				...
				<svg
					aria-hidden="true"
					class="block w-full h-full {spinnerClasses}"
					viewBox="0 0 100 101"
					fill="none"
			...
			{:else}
				<svg
					aria-hidden="true"
					class="block w-full h-full animate-spin {spinnerClasses}"
					viewBox="0 0 100 101"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
				>
...
{:else}
	<div
		class="{clazz} select-none transition-colors flex justify-center px-3 py-2 rounded-md text-sm font-semibold shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed {done
			? ''
			: themeClassesStatic} {themeClasses}"
	>
		<!-- <span class="flex-none"></span> -->
