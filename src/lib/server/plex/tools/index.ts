...
export const search = async (
	...
	plexToken: string,
	debug = 0
): Promise<null | types.SearchResult> => {
	const results = await resource(
		`/library/sections/${sectionKey}/all`,
		types.Method.GET,
		{ type: type.toString(), title },
		plexURL,
		plexToken,
		debug
	);
...
export const selectBestResult = (
	results: types.SearchResult | null,
	book: types.BookType,
	debug = 0
): types.SearchMetadata | null => {
	// Exit if we don't have the required info
	if (
		results === null ||
		results.MediaContainer.size === 0 ||
		results.MediaContainer.Metadata.length === 0
	)
		return null;
	...
	for (const option of results.MediaContainer.Metadata) {
		...
		if (debug > 1)
			console.log(
				option.title.toLocaleLowerCase().trim(),
				'===',
				title,
				'=',
				option.title.toLocaleLowerCase().trim() === title
			);
			...
			for (const author of authors) {
				if (debug > 1)
					console.log(
						author,
						'===',
						option.parentTitle.toLocaleLowerCase().trim(),
						'=',
						author === option.parentTitle.toLocaleLowerCase().trim()
					);
	...
	const searchResults = new Fuse(results.MediaContainer.Metadata, fuseOptions)
		.search(book.title)
		.map((v) => v.item);
...
export const resource = async (
	...
	debug = 0,
	parseJSON = true
): Promise<types.ResourceResult> => {
	// Make sure the params make sense
	if (plexToken === '' || plexURL === '' || plexToken === undefined || plexURL === undefined)
		return null;
	...
	try {
		...
		if (debug && response.status === 401)
			console.log('unauthorized', response.status, response.statusText, plexToken);
 ...
 */
export const getServerIdentity = async (
	plexURL: string,
	plexToken: string
): Promise<null | types.IdentityResult> => {
 ...
 */
export const matchBookToPlexEntry = async (
	asin: string,
	plexURL?: string,
	plexToken?: string,
	recursionDepth = 0
	...
	const book: types.BookType | null = await prisma.book.findUnique({
		where: { asin: asin },
	...
	if (author !== null) {
		...
		if (author.plexKey === null) {
			...
			const authorSearch = await search(
				sectionKey,
				author.name,
			...
			if (
				authorSearch !== null &&
				authorSearch.MediaContainer.size > 0 &&
				authorSearch.MediaContainer.Metadata.length > 0
			) {
	...
	if (authorKey !== null) {
		...
		const authorLookup = await resource(
			`/library/metadata/${authorKey}`,
			types.Method.GET,
			{},
			plexURL,
			plexToken,
			debug
		); // as unknown as null | types.SearchResult;
		if (
			authorLookup !== null &&
			authorLookup.status === 200 &&
			authorLookup.value !== null &&
			(authorLookup.value as types.SearchResult).MediaContainer.size > 0 &&
			(authorLookup.value as types.SearchResult).MediaContainer.Metadata.length > 0 &&
			(authorLookup.value as types.SearchResult).MediaContainer.Metadata[0].type === 'artist'
		) {
			if (debug) console.log('author validated', authorKey);
			// We have an author key and the author exists in Plex. Good start.
			const bookLookup = await resource(
				`/library/metadata/${authorKey}/children`,
				types.Method.GET,
				{
					title: book.title,
			...
			const selectedBook = selectBestResult(
				bookLookup?.value as null | types.SearchResult,
				book,
				debug
			);
