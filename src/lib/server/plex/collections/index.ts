 ...
 */
export const createCollection = async (title: string, sectionId: string, plexURL: string, plexToken: string, debug=0): Promise<string | null> => {
  // Make the collection
  const results = await tools.resource('/library/collections', types.Method.POST, {
 ...
 */
export const deleteCollection = async (plexKey: string, plexURL: string, plexToken: string, debug=0): Promise<boolean> => {
  // Make the collection
  const results = await tools.resource(`/library/collections/${plexKey}`, types.Method.DELETE, {}, plexURL, plexToken, debug, false);
  // Exit if it failed
...
export const checkCollectionExists = async (collectionKey: string, plexURL: string, plexToken: string, debug=0): Promise<boolean> => {
  ...
  const collectionLookup = await tools.resource(`/library/collections/${collectionKey}`, types.Method.GET, {}, plexURL, plexToken, debug);
...
export const getCollectionThumbnailURLs = async (collectionKeys: string[], plexURL: string, plexToken: string): Promise<{ [key: string]: string } | null> => {
  ...
  for (const collectionKey of collectionKeys) promiseList.push(tools.resource(`/library/collections/${collectionKey}`, types.Method.GET, {}, plexURL, plexToken, debug));
 ...
 */
export const collectBySeries = async (series: types.SeriesType, plexURL: string, plexToken: string, debug=0): Promise<boolean> => {
  ...
  if (series.plexKey !== null) {
    // We need to verify that the collection exists in Plex also
    const collectionLookup = await tools.resource(`/library/collections/${series.plexKey}`, types.Method.GET, {}, plexURL, plexToken, debug);
    if (collectionLookup !== null && collectionLookup.status === 200 && collectionLookup.value !== null && (collectionLookup.value as types.SearchResult).MediaContainer.size > 0 && (collectionLookup.value as types.SearchResult).MediaContainer.Metadata.length > 0) {
      ...
      if ((collectionLookup.value as types.SearchResult).MediaContainer.Metadata[0].title !== series.title) {
        ...
        await tools.resource(`/library/collections/${series.plexKey}`, types.Method.PUT, { 'title': series.title }, plexURL, plexToken, debug, false);
      }
    } else {
  ...
  const results = await tools.resource(`/library/collections/${collectionKey}/items`, types.Method.PUT, {
 ...
 * @returns whether or not the collection was made and books were added
 */
export const collectToSpecificCollection = async (books: types.BookType[], collectionKey: string, plexURL: string, plexToken: string, debug=0): Promise<boolean> => {
