...
export const exec = async (
	...
	// Exit if no authcode
	if (authCode === undefined || cli_id === undefined)
		return ConversionError.NO_PROFILE_WITH_AUTHCODE;

	const LIBRARY_FOLDER = await settings.get('library.location');

	// Delete this book from the downloaded library if it exists
	const destinationFolder = `${LIBRARY_FOLDER}/${sanitizeFile(book.authors[0].name)}/${sanitizeFile(
		book.title
	)}`;
	...
	await prisma.processQueue.update({
		where: { id: processID },
		data: { book: { update: { process_progress: 0 } } }
	});
	...
	const args = [
		...
		authCode,
		'--dir-naming-scheme',
		`'"${sanitizeFile(book.authors[0].name)}/${sanitizeFile(book.title)}"'`,
		'--file-naming-scheme',
		`'"${sanitizeFile(book.title)}"'`,
	...
	const aax = child_process.spawn(AAXtoMP3_COMMAND, args, {
		cwd: tmpDir,
		detached: true,
		shell: true
	});
	...
	const promise = new Promise<ConversionError>((resolve, reject) => {
		...
		const dataProcessorLoading = async (d: Buffer) => {
			...
			const regex =
				/size=\s+(?<size>[0-9]+)kB\s+time=(?<hr>[0-9][0-9]):(?<min>[0-9][0-9]):(?<sec>[0-9][0-9].[0-9][0-9])\s+bitrate=\s+(?<bitrate>[0-9]+.[0-9])kbits\/s\s*speed=\s*(?<speed>[0-9.e+]+)/;
			...
			if (groups !== undefined && bookRuntime_s !== 0) {
				const bookTime_s =
					parseInt(groups.hr) * 3600 + parseInt(groups.min) * 60 + parseFloat(groups.sec);
	...
	if (results === ConversionError.NO_ERROR) {
		try {
			...
			await media.saveFile(
				`${destinationFolder}/${sanitizeFile(book.title)}.m4b`,
				book.asin,
				sourceId,
				{
