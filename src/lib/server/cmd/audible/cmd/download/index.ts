...
export const download = async (
	...
	processID: string,
	tmpDir: string
): Promise<{ e: BookDownloadError; sourceId?: string }> => {
	...
	if (cli_id === undefined || sourceId === undefined)
		return { e: BookDownloadError.NO_PROFILE_WITH_AUTHCODE };
	...
	const promise = new Promise<BookDownloadError>((resolve, reject) => {
		...
		const dataProcessorLoading = async (d: Buffer) => {
			...
			const regex =
				/(?<percent>[0-9]+)%\|[█▉▊▋▌▍▎▏\s]+\| (?<downloaded>[0-9.]+)(?<downloadUnit>[MG])\/(?<total>[0-9.]+)(?<totalUnit>[MG]) \[.+ (?<speed>[0-9.]+)[a-zA-Z]+\/s]/;
		...
		if (book.cover !== null) {
			...
			if (foundImage === false) {
				...
				if (book.cover.url_500.indexOf('ui-avatars.com')) {
					...
					cover = helpers.toBuffer(
						await (await fetch(book.cover.url_500 + '&format=png')).arrayBuffer()
					);
				} else {
					// Just grab the image
		...
		for (const jsonFile of jsonFiles) {
			try {
				if (jsonFile.endsWith('chapters.json')) {
					const chapterObj = JSON.parse(
						fs.readFileSync(tmpDir + '/' + jsonFile, 'utf8')
					) as AmazonChapterData;
