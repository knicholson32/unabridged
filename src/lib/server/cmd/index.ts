...
export namespace LibraryManager {
	...
	const sendInvalidate = async (typeFilter = types.ProcessType.BOOK, tag?: string) => {
		if (tag !== undefined) console.log(tag);
		events.emit(
			'processor.invalidate',
			(await prisma.processQueue.count({ where: { type: typeFilter, is_done: false } })) > 0
		);
	};
	...
	const getQueueFinder = () => {
		return {
			...
			where: {
				AND: [
					{
						OR: [{ try_after_time: null }, { try_after_time: { lte: Date.now() } }]
					},
					{ in_progress: false },
					{ is_done: false }
				]
	...
	let processorPromises: (Promise<void> | undefined)[] = Array.from(
		{ length: CONCURRENT_PROCESSES },
		(_, i) => undefined
	);
		...
		while (queueEntry !== null && (await settings.get('progress.paused')) === false) {
			...
			if (type === 'BOOK' && queueEntry.book !== null) {
				...
				try {
					...
					const { e: bookDownload, sourceId: sourceId } = await audible.cmd.download.download(
						queueEntry.book.book.asin,
						queueEntry.id,
						tmpDir
					);
					...
					switch (bookDownload) {
						...
						case BookDownloadError.NO_PROFILE_WITH_AUTHCODE:
							...
							if (targetBook !== undefined && targetBook !== null) {
								...
								for (const s of targetBook.sources)
									if (
										s.type === types.SourceType.AUDIBLE &&
										s.audible !== null &&
										s.audible.activation_bytes === null
									)
										await audible.cmd.profile.fetchMetadata(s.id, false);
								...
								if (book !== null) {
									...
									if (bytes === true) {
										await tryAgainLater(
											queueEntry.id,
											type,
											ATTEMPT_COOLDOWN_SHORT,
											ProcessError.NO_PROFILE_WITH_AUTHCODE
										);
										break;
									}
								}
						...
						default:
							...
							await processFailed(
								queueEntry.id,
								type,
								bookDownload as unknown as ProcessError,
								0,
								0
							);
							break;
					...
					if (bookDownload === BookDownloadError.NO_ERROR && sourceId !== undefined) {
						...
						await prisma.processQueue.update({
							where: { id: queueEntry.id },
							data: { result: null, book: { update: { download_progress: 1 } } }
						});
						// 3. Process the book, updating the book progress as required
						const bookProcess = await AAXtoMP3.cmd.convert.exec(
							queueEntry.book.book.asin,
							sourceId,
							queueEntry.id,
							tmpDir
						...
						switch (bookProcess) {
							...
							case ConversionError.DESTINATION_NOT_WRITABLE:
								...
								await processFailed(
									queueEntry.id,
									type,
									ProcessError.DESTINATION_NOT_WRITABLE,
									1,
									0
								);
					...
					if (noneWorking && (await settings.get('progress.running')) === true) {
						...
						if (libraryScanShouldBeIssued) {
							const plexSettings = await settings.getMany(
								...
								'plex.library.autoScan.enable',
								'plex.library.autoScan.scheduled'
							);
							console.log(plexSettings);
							if (
								plexSettings['plex.enable'] === true &&
								plexSettings['plex.collections.enable'] === true &&
								plexSettings['plex.library.autoScan.enable'] === true &&
								plexSettings['plex.library.autoScan.scheduled'] == false
							)
								Plex.triggerAutoScan();
						}
	...
	export const start = async () => {
		...
		if (global.manager === undefined)
			global.manager = { interval: undefined, cronTask: undefined, runProcess: runProcess };
	...
	export const queueBook = async (asin: string, run = true) => {
		try {
			if (!(await getIsQueued(asin))) {
				...
				for (let i = 0; i < 15 && (await prisma.processQueue.count({ where: { id } })) > 0; i++)
					id = uuidv4().substring(0, i + 1);
				...
				const queued = await prisma.processQueue.findUnique({
					include: validators.processQueueBOOKInclude,
					where: { id: id, type: types.ProcessType.BOOK }
				});
				console.log('queued', queued);
				if (queued !== null) await sendInvalidate();
			}
	...
	export const removeBook = async (
		asin: string,
		result: ProcessError = ProcessError.CANCELED,
		...
		try {
			...
			const eventProcess = await prisma.processQueue.findFirst({
				where: { book: { bookAsin: asin } }
			});
			if (eventProcess !== null) await sendInvalidate();
...
export namespace Cron {
	...
	const checkCronTime = async (debug?: boolean) => {
		const shouldExit =
			Math.floor(Date.now() / 1000) - lastCronStartTime >=
			(await settings.get('system.cron.maxRun'));
		if (debug === true)
			console.log(
				`cron time: ${Math.floor(Date.now() / 1000)} : ${lastCronStartTime} | ${
					shouldExit ? 'Cron will exit' : 'Cron will continue'
				}`
			);
		return shouldExit;
	};
	...
	const process = async () => {
		try {
			...
			if ((await settings.get('general.autoSync')) === true) {
				...
				for (const source of sources) {
					...
					if (source.type === types.SourceType.AUDIBLE) {
						...
						if (debug)
							console.log(
								`Source '${source.id}' was not synced because type '${source.type}' is not implemented.`
							);
					}
			...
			if (
				(await settings.get('plex.library.autoScan.enable')) === true &&
				(await settings.get('plex.enable')) === true &&
				(await settings.get('plex.library.key')) !== ''
			) {
				...
				cronRecord.scanLibrary.success =
					results === types.ScanAndGenerate.NO_ERROR ||
					results === types.ScanAndGenerate.NO_ERROR_COLLECTIONS_DISABLED;
	...
	export const start = async () => {
		...
		if (!cron.validate(cronString)) {
			if (debug)
				console.log('Cron failed to start because the cron string is invalid:', cronString);
			return false;
		}
