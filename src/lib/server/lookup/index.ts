...
const getISBN = (
	data: { type: 'ISBN_10' | 'ISBN_13'; identifier: string }[]
): string | undefined => {
...
export const saveGoogleAPIDetails = async (asin: string, title: string, authors: string[]) => {
	...
	try {
		...
		const description = primaryResult.volumeInfo.description
			.replace(/\.([^\.\s])/g, '. $1')
			.replaceAll('.  ', '. ');
		const searchDescription = primaryResult.searchInfo.textSnippet
			.replace(/\.([^\.\s])/g, '. $1')
			.replaceAll('.  ', '. ');
