...
create:
	docker build --file ./docker/Dockerfile --target prod -t keenanrnicholson/unabridged .
	docker push keenanrnicholson/unabridged
create-all:
    # buildx create --name mybuilder && --platform linux/arm64,linux/amd64
	docker buildx build --builder mybuilder --file ./docker/Dockerfile --push --platform linux/arm64 --tag keenanrnicholson/unabridged:latest .
setup:
