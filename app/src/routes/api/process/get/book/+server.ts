...
import { API, type ProcessQueueBOOK, processQueueBOOKValidator } from '$lib/types';
import { compressJSON } from '$lib/server/helpers';

export const GET = async ({ request, setHeaders }) => {

  const processes: ProcessQueueBOOK[] = await prisma.processQueue.findMany(processQueueBOOKValidator);
