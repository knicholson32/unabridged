...
const restoreBase = () => {
  ...
  for (const e of Event.Base.Names) for (const callback of callbackMapBase[e]) evtSourceBase.addEventListener(e, callback);
}

// Create a function to re-attach all callbacks when the EventSource is created or re-created
const restoreProgress = () => {
  if (evtSourceProgress !== null) for (const e of Event.Progress.Names) for (const callback of callbackMapProgress[e]) evtSourceProgress.addEventListener(e, callback);
}
 ...
 */
export const on = <T extends Event.Base.Name>(event: T, callback: (data: Event.Base.Type<T>) => void) => {
