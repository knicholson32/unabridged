...
export const scanLibraryFiles = async (progressCallback?: (progress: number) => void): Promise<publicTypes.ScanAndGenerate> => {
  ...
  return new Promise(async (resolve) => {
    ...
    if (plexSettings['plex.enable'] !== true) return resolve(publicTypes.ScanAndGenerate.PLEX_DISABLED);
    if (plexSettings['plex.library.autoScan.enable'] !== true) return resolve(publicTypes.ScanAndGenerate.AUTO_SCAN_DISABLED);
    ...
    const scan = new Promise<publicTypes.ScanAndGenerate>((resolve) => {
      ...
      ws.on('open', async () => {
        ...
        const result = await tools.resource(`/library/sections/${sectionId}/refresh`, types.Method.GET, {}, plexURL, plexToken, debug, false);
  ...
  try {
    ...
    if (plexSettings['plex.collections.by'] === publicTypes.CollectionBy.series) {
      ...
      if (plexSettings['plex.collections.groupSingles'] === true && books.length > 0) {
        ...
        if (key === '') {
          key = await collections.createCollection('No Series', plexSettings['plex.library.key'], plexSettings['plex.address'], plexSettings['plex.token'], debug);
          await settings.set('plex.collections.singlesKey', key ?? '');
        }
        // Check to see that the key worked
        if (key !== null && key !== '' && await collections.checkCollectionExists(key, plexSettings['plex.address'], plexSettings['plex.token'], debug)) {
          console.log('Adding books', books);
          // Add all the books to the collection
          await collections.collectToSpecificCollection(books, key, plexSettings['plex.address'], plexSettings['plex.token'], debug);
        }
      } else {
        // We should not. Delete it if it exists
        if (plexSettings['plex.collections.singlesKey'] !== '')
          if (await collections.deleteCollection(plexSettings['plex.collections.singlesKey'], plexSettings['plex.address'], plexSettings['plex.token'], debug))
