<script lang="ts">
	import type { Progress, ProgressAPI, ProgressStatus } from "$lib/types";
  ...
  const updateProgress = (p: Progress) => {
    progress = p;
    if (progress.status as ProgressStatus === 'DONE' || progress.status as ProgressStatus === 'ERROR') intervalSlow();
    else intervalFast();
  }
  ...
  $: offset = circumference - (progress?.progress ?? 0) * circumference
  ...
  <div class="relative flex-none ml-2 w-5 h-5 align-middle overflow-hidden">
    {#if progress !== undefined}
      {#if progress.progress !== 0}
        ...
        <span class="sr-only">Loading... {Math.floor(progress.progress * 100)}%</span>
      {:else}
